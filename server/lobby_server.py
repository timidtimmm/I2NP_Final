...
def handle_list_games(payload):
    """列出遊戲 - 只顯示檔案系統中實際存在且 active 的遊戲（需登入）"""
    _, err = require_player(payload)
    if err:
        return err
...
# ----------------- Room Liveness Monitor ----------------- #

def is_room_alive(host, port):
    """
    Lobby 端判斷房間是否還活著：
    1) 先試 rooms.json 記錄的 host（通常是 PUBLIC_HOST）
    2) 再退回試 127.0.0.1（避免本機連 public IP 誤判）
    """
    targets = []
    if host:
        targets.append(host)
...
def room_liveness_monitor_loop(stop_event):
    """
    每 2 秒掃一次 rooms.json
    - 房間 server 連不到 → 直接刪房
    """
    while not stop_event.is_set():
        ...
        for rid, r in list(rooms.items()):
            ...
            if not is_room_alive(host, port):
                # 房間死了（常見原因：GameServer Ctrl+C）
                rooms.pop(rid, None)
                changed = True
...
def start_room_liveness_monitor(stop_event):
    t = threading.Thread(
        target=room_liveness_monitor_loop,
        args=(stop_event,),
        daemon=True
    )
    t.start()
